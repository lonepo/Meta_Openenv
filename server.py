#!/usr/bin/env python3
"""
server.py — FastAPI HTTP server wrapping CircuitSynthEnv.

Exposes the environment as a stateful REST API so that:
  - HuggingFace Spaces can deploy it as a Docker service
  - openenv validate can ping it
  - The inference script can connect via HTTP (optional; in-process also works)

Endpoints:
  GET  /health          → {"status": "ok", "env": "CircuitSynth-SquareWave"}
  POST /reset           → ResetResponse
  POST /step            → StepResponse
  GET  /state           → StateResponse
  GET  /tasks           → list of task specs

Session management: each client gets a session_id; multiple concurrent
episodes are supported via session-keyed env instances.
...
def _format_obs_text(env: CircuitSynthEnv) -> str:
    """Text observation for LLM consumption."""
    task    = env.task
...
class StepRequest(BaseModel):
    ...
    action:     Dict[str, Any] = Field(
        ...,
        description=(
            "Structured action dict. Examples: "
...
def _parse_action_dict(obj: dict) -> np.ndarray:
    """Convert a dict action (from HTTP body) to env's int array."""
    ...
    if act in ("ADD", "ADD_COMPONENT"):
        ...
        val_idx   = int(obj.get("value_idx", 10))
